...
var bannedIPs = {};
var blacklistedIPs = {'120.36.210.254': 0,
		      '68.13.124.117': 0};
...
function IsBannedIp(ip){
    if (blacklistedIPs[ip]) return true;
...
function processShare(miner, job, blockTemplate, nonce, resultHash){
	...
	log('info', logSystem, 'majorVersion::%s', [majorVersion]);
	log('info', logSystem, 'shareBuffer::%s', [shareBuffer[1]]);
        hash = cryptoNight(convertedBlob, 1);
        shareType = 'valid';
    }

    log('info', logSystem, 'hash1::%s', [resultHash]);
